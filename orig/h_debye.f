C
C     COPYRIGHT:  Copyright 1998-2003
...
c      write(3,*) ' alpha = ',alpha
      alphar = dreal(alpha)
      alphai = dimag(alpha)
      thalpa = zsinh(alpha)/zcosh(alpha)
      snhalp = zsinh(alpha)
c      write(3,*) ' alpha = ',alpha,' sinh(alpha) = ',snhalp
      denom = zsqrt(dpi*arg/2.0d0)*zsqrt(-dj*zsinh(alpha))
c      write(3,*) ' arg = ',arg
c      write(3,*) ' denom = ',denom
      if (dabs(dreal(nu*(thalpa-alpha))) .gt. 690.d0) then
        ierror = 1
        go to 10
      end if
...
c      write(3,*) ' s2 = ',s2
      if (indexr .eq. 0) then
        ...
        if ((dabs(dimag(nu)) .lt. 1.0d-08) .and.
     *      (dabs(dmod(dreal(nu),1.0d0)) .lt. 1.0d-08)) then
     ...
     *             ((1.0d0+zexp(-dj*(morn+1)*nu*dpi)*sinrat)*s2+s1)
        ...
        if ((dabs(dimag(nu)) .lt. 1.0d-08) .and.
     *      (dabs(dmod(dreal(nu),1.d0)) .lt. 1.0d-08)) then
          sinrat = dcmplx(dfloat(morn),0.0d0)
        else
          sinrat = zsin(morn*nu*dpi)/zsin(nu*dpi)
        end if
