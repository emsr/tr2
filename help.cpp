// /home/ed/bin/bin/g++ -std=c++11 -fdiagnostics-color=auto -o help help.cpp
...
main()
{
  ...
  ss.seekg(0);
  ss.clear();
  ss << quoted(_A(original));
  std::cout << "_A(original): " << ss.str() << '\n';
  ss >> quoted(a_round_trip); // Works with int& quoted(_A& __a).
  std::cout << "a_round_trip: " << a_round_trip << '\n';
  ss >> quoted(_A(round_trip)); // Works with int& quoted(_A&& __a).
  std::cout << "quoted(_A(round_trip)): " << quoted(_A(round_trip)) << '\n';
}
