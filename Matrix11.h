...
template<typename T = double, int D = 1>
  ...
  private:
    ...
    //  template<typename A> Matrix(A);
  };

template<typename T = double, int D = 1> class Row;
...
template<typename T>
  ...
  public:
    Matrix_base(Index n)
    : elem(new T[n]()), sz(n), owns(true), xfer(false)
    ...
    : elem(p), sz(n), owns(false), xfer(false)
    { }
    ...
    void
    copy_elements(const Matrix_base & a)
    {
      if (sz != a.sz)
        error("copy_elements()");
    ...
    void
    base_assign(const Matrix_base & a)
    { copy_elements(a); }

    void
    base_copy(const Matrix_base & a)
    {
      if (a.xfer)
...
private:
    ...
    void
    operator=(const Matrix_base &);    // no ordinary copy of bases
...
//-----------------------------------------------------------------------------
  ...
  protected:
    // for use by Row:
    Matrix(Index n1, T * p)
    : Matrix_base<T>(n1, p), d1(n1)
    { }

  public:

    Matrix(Index n1)
    : Matrix_base<T>(n1), d1(n1) { }

    Matrix(Row<T, 1> & a)
    : Matrix_base<T>(a.dim1(), a.p), d1(a.dim1()) 
    { 
    ...
    : Matrix_base<T>(a.size(),0), d1(a.d1)
    {
        // std::cerr << "copy ctor\n";
        this->base_copy(a);
    }
    ...
    Matrix
    xfer()    // make an Matrix to move elements out of a scope
    {
    ...
    T &
    operator()(Index n1)
    { range_check(n1); return this->elem[n1]; }

    const T &
    operator()(Index n1) const
    { range_check(n1); return this->elem[n1]; }
...
template<typename T>
  ...
  public:
    ...
    //  make an Matrix to move elements out of a scope
    Matrix
    xfer()
    {
    ...
    void
    range_check(Index n1, Index n2) const
    {
      if (n1 < 0 || d1 <= n1)
    ...
    //  subscripting:
    T &
    operator()(Index n1, Index n2)
    { range_check(n1, n2); return this->elem[n1 * d2 + n2]; }

    const T &
    operator()(Index n1, Index n2) const
    { range_check(n1, n2); return this->elem[n1 * d2 + n2]; }
...
template<typename T>
  ...
  public:
    ...
    void
    range_check(Index n1, Index n2, Index n3) const
    {
      if (n1 < 0 || d1 <= n1)
    ...
    // subscripting:
    T &
    operator()(Index n1, Index n2, Index n3)
    {
    ...
    const T &
    operator()(Index n1, Index n2, Index n3) const
    {
      range_check(n1, n2, n3);
      return this->elem[d2 * d3 * n1 + d3 * n2 + n3];
    }
...
template<typename T>
  Matrix<T>
  scale_and_add(const Matrix<T> & a, T c, const Matrix<T> & b)
  {
    if (a.size() != b.size())
...
template<typename T>
  ...
  public:
    ...
    Matrix<T, 2> &
    operator=(const T & c)
    { this->base_apply(Assign<T>(), c); return *this; }

    Matrix<T, 2> &
    operator=(const Matrix<T, 2> & a)
...
template<typename T, int N>
  Matrix<T, N - 1>
  scale_and_add(const Matrix<T, N> & a, const Matrix<T, N - 1> c, const Matrix<T, N - 1> & b)
  {
    Matrix<T> res(a.size());
    if (a.size() != b.size())
      error("sizes wrong for scale_and_add");
...
template<typename T, int D>
  Matrix<T, D>
  operator*(const Matrix<T, D> & m, const T & c)
  {
    Matrix<T, D> r(m);
    return r *= c;
  }
