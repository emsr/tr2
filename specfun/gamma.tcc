...
namespace __detail
{
  ...
  template<typename _Tp>
    ...
    {
      const double _S_eps = 3.0 * std::numeric_limits<_Tp>::epsilon();
  ...
  template<typename _Tp>
    ...
    {
      constexpr _Tp _S_fpmin = 3.0 * std::numeric_limits<_Tp>::min();
      constexpr _Tp _S_eps = 3.0 * std::numeric_limits<_Tp>::epsilon();
      constexpr unsigned int __itmax = 10 * (10 + std::sqrt(std::abs(__a)));
  ...
  template<typename _Tp>
    ...
    __psi(_Tp __x)
    {
      ...
      const _Tp _S_eps = _Tp(4) * std::numeric_limits<_Tp>::epsilon();
